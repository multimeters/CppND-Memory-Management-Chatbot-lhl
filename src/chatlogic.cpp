...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //std::vector<GraphNode> n_=*;
                        auto newNode = std::find_if(_nodes.begin(),_nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            std::unique_ptr<GraphNode> s=std::make_unique<GraphNode>(id);
                            //_nodes.emplace_back(new GraphNode(id));
                            _nodes.push_back(std::move(s));
     
                            newNode = _nodes.end()-1; // get iterato to last element
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //std::unique_ptr<GraphEdge> edge1=std::make_unique<GraphEdge>(id);
    ...
    ChatBot mchatBot("../images/chatbot.png");
    SetChatbotHandle(&mchatBot);
    mchatBot.SetChatLogicHandle(this);
    mchatBot.SetRootNode(rootNode);

    rootNode->MoveChatbotHere(std::move(mchatBot));
